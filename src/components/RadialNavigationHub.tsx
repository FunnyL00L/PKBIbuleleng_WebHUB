...
export const RadialNavigationHub: React.FC<RadialNavigationHubProps> = ({
  ...
  const [orbitRadius, setOrbitRadius] = useState<number>(185);
    ...
    <div
      className={`relative w-full flex flex-col items-center justify-center overflow-hidden transition-colors ${
        isOverlay
          ...
          : isLight
          ? 'min-h-[85vh] py-12 px-4 bg-gradient-to-b from-white via-blue-50/40 to-slate-100 text-slate-800'
          : 'min-h-[85vh] py-12 px-4 bg-gradient-to-b from-[#060c1c] via-[#091329] to-[#0c1833] text-white'
      }`}
    >
      ...
      <div className="absolute top-6 right-6 z-40 flex items-center gap-2">
        <button
          ...
          className={`flex items-center gap-2 px-3.5 py-2 rounded-2xl border transition-all shadow-sm cursor-pointer hover:scale-105 active:scale-95 ${
            isLight
        ...
        >
          <BulelengLogo size={28} className="drop-shadow-xs" />
      ...
      <div className="absolute inset-0 pointer-events-none flex items-center justify-center">
        <div
          className={`w-[520px] h-[520px] rounded-full blur-3xl transition-all duration-700 ${
            isLight
              ? isExpanded
      ...
      {/* Top Banner Text */}
      <div className="relative z-20 text-center max-w-2xl mb-8 sm:mb-12 px-4">
        <div
        ...
        <p
          className={`mt-1 text-base sm:text-xl font-semibold tracking-wide ${
        ...
        <p
          className={`mt-2 text-xs sm:text-sm leading-relaxed max-w-lg mx-auto ${
      ...
      >
        ...
        >
          ...
          {isExpanded &&
            NAV_ITEMS.map((item) => {
              ...
              const x = 50 + (orbitRadius * Math.cos(rad) * 100) / (orbitRadius * 2 + 160);
              const y = 50 + (orbitRadius * Math.sin(rad) * 100) / (orbitRadius * 2 + 160);
        ...
        <div className="relative flex flex-col items-center">
          <motion.button
            ...
            whileTap={{ scale: 0.94 }}
            className={`relative group rounded-full p-3 transition-all duration-300 cursor-pointer shadow-2xl focus:outline-none ${
              isExpanded
          ...
          >
            <BulelengLogo size={window.innerWidth < 640 ? 84 : 110} glow={isExpanded} />

            {/* Tap prompt indicator badge */}
            <span
              className={`absolute -bottom-3 left-1/2 -translate-x-1/2 whitespace-nowrap text-[11px] font-bold px-3.5 py-1 rounded-full shadow-lg transition-all ${
                isExpanded
              ...
              return (
                ...
                >
                  <motion.button
                    ...
                    whileTap={{ scale: 0.92 }}
                    className={`relative w-[84px] h-[84px] sm:w-[94px] sm:h-[94px] rounded-full flex flex-col items-center justify-center p-2 text-white shadow-2xl transition-all cursor-pointer group focus:outline-none ${
                      isActive
                  ...
                  >
                    {/* Background gradient */}
                    <div
                      className={`absolute inset-0 rounded-full bg-gradient-to-br ${item.color} opacity-95 group-hover:opacity-100 transition-opacity`}
                    />
                    ...
                    <div className="relative z-10 flex flex-col items-center text-center">
                      <Icon className="text-white drop-shadow mb-1" size={24} />
                      <span className="text-[10px] sm:text-[11px] font-bold leading-tight line-clamp-1 max-w-[76px]">
                    ...
                    <AnimatePresence>
                      {hoveredItem?.id === item.id && (
                        <motion.div
                          ...
                          className={`absolute z-40 w-56 sm:w-64 p-3.5 rounded-2xl bg-slate-900/95 text-white shadow-2xl border border-blue-500/70 backdrop-blur-md pointer-events-none ${
                            targetY > 0 ? '-top-32 sm:-top-36' : '-bottom-32 sm:-bottom-36'
                          } ${targetX > 0 ? '-left-20' : '-right-20'}`}
                        >
